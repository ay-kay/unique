...
public class ErrorMessageService {
	...
	private static Logger fsLogger = Logger.getLogger(ErrorMessageService.class);

	@POST
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public Response storeErrorMessage(@HeaderParam("Authorization") String hmac,
			String input) {
		...
		if (!myHMAC.equals(hmac) && !myHMAC.equals(defaultHMAC)) {
			MDC.put("input", input);
			mongoLogger.error("Authorization failed for error message request");
			MDC.remove("input");
		...
		boolean success = mongoConnector.saveErrorMessage(input);
		if (success) { 
			mongoLogger.error("neue ErrorMessage eingegangen");
		}
